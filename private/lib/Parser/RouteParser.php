...
/**
 * RAVEN CMS
 * ~/private/lib/Parser/RouteParser.php
...
final class RouteParser
{
    ...
    private ChannelParser $channel;
    private GroupParser $group;
    ...
    public function __construct(Config $config, InputSanitizer $input)
    {
        $this->config = $config;
        $this->input = $input;
        $this->channel = new ChannelParser($config, $input);
        $this->group = new GroupParser($config, $input);
        $this->feed = new FeedParser($config, $input);
    }
