...
$customGithubKey = 'custom-github';
$customGitRepoKey = 'custom-git-repo';
$isCustomSource = in_array($sourceKey, [$customGithubKey, $customGitRepoKey], true);
$customSourceLabel = $sourceKey === $customGithubKey ? 'Custom GitHub' : 'Custom Git Repo';
$customSourcePlaceholder = $sourceKey === $customGithubKey
    ? 'owner/repo (for example: noveltylanterns/raven)'
    : 'https://git.example.com/owner/repo.git or git@example.com:owner/repo.git';
...
<header class="card">
    <div class="card-body">
        <h1>Update System</h1>
        <p class="text-muted">Update your Raven installation from one of our stock mirrors, or enter a custom source.</p>
        <div class="d-flex flex-wrap align-items-center gap-2">
            ...
            <select class="form-select form-select-sm w-auto" id="update-source-key" name="source_key" aria-label="Upstream Source">
                ...
                ?>
                <option value="<?= e($optionKey) ?>"<?= $optionKey === $sourceKey ? ' selected' : '' ?>><?= e($optionLabel) ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="mt-2<?= $isCustomSource ? '' : ' d-none' ?>" id="updater-custom-repo-wrap">
            <label for="updater-custom-repo" class="form-label mb-1" id="updater-custom-repo-label"><?= e($customSourceLabel) ?></label>
            <input
              id="updater-custom-repo"
              type="text"
              class="form-control form-control-sm"
              value="<?= e($customRepo) ?>"
              placeholder="<?= e($customSourcePlaceholder) ?>"
              >
        </div>
    </div>
</header>
    ...
    <div class="card-body">
        ...
        <div class="row g-3">
            <div class="col-12 col-md-6">
                <div class="mb-3">
                    <label class="form-label">Local Revision</label>
                    <div><code><?= e($currentRevision !== '' ? $currentRevision : '<unknown>') ?></code></div>
                </div>
                <div class="mb-3">
                    <label class="form-label">Local Branch</label>
                    <div><code><?= e($localBranch !== '' ? $localBranch : '<unknown>') ?></code></div>
                </div>
            </div>
            <div class="col-12 col-md-6">
                <div class="mb-3">
                    <label class="form-label">Latest Upstream Revision</label>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h2 id="updaterForceRunModalLabel">Force Run Updater?</h2>
...
<script>
  (function () {
    ...
    var customRepoLabel = document.getElementById('updater-custom-repo-label');
    var customRepoInput = document.getElementById('updater-custom-repo');
    var customGithubKey = '<?= e($customGithubKey) ?>';
    var customRepoKey = '<?= e($customGitRepoKey) ?>';
    ...
    var syncCustomSourceFieldUi = function () {
      var isCustomGithub = sourceSelect.value === customGithubKey;
      var isCustomRepo = sourceSelect.value === customRepoKey;
      var showCustom = isCustomGithub || isCustomRepo;
      if (customRepoWrap instanceof HTMLElement) {
        customRepoWrap.classList.toggle('d-none', !showCustom);
      }
      if (customRepoInput instanceof HTMLInputElement) {
        customRepoInput.disabled = !showCustom;
        customRepoInput.placeholder = isCustomGithub
          ? 'owner/repo (for example: noveltylanterns/raven)'
          : 'https://git.example.com/owner/repo.git or git@example.com:owner/repo.git';
      }
      if (customRepoLabel instanceof HTMLElement) {
        customRepoLabel.textContent = isCustomGithub ? 'Custom GitHub' : 'Custom Git Repo';
      }
    };
    var syncCustomRepoVisibility = function () {
      syncCustomSourceFieldUi();
    };
