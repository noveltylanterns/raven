...
$bulkDeleteFormId = 'bulk-categories-delete-form';
$categoriesTableId = 'categories-table';
$categoriesSearchId = 'categories-filter-search';
$categoriesCountId = 'categories-filter-count';
$categoriesEmptyId = 'categories-filter-empty';
$pagination = is_array($pagination ?? null) ? $pagination : [];
$paginationCurrent = max(1, (int) ($pagination['current'] ?? 1));
$paginationTotalPages = max(1, (int) ($pagination['total_pages'] ?? 1));
$paginationTotalItems = max(0, (int) ($pagination['total_items'] ?? count($categories)));
$paginationBasePath = (string) ($pagination['base_path'] ?? ($panelBase . '/categories'));
...
<div class="card mb-3">
    <div class="card-body">
        ...
        <p class="text-muted mt-2 mb-0">Manage category taxonomy used to organize pages and category landing views.</p>
    </div>
</div>

<!-- Standalone bulk-delete form receives selected row ids via `form` attribute. -->
<form id="<?= e($bulkDeleteFormId) ?>" method="post" action="<?= e($panelBase) ?>/categories/delete">
    <?= $csrfField ?>
</form>
        ...
        <?php else: ?>
            <div class="row g-2 mb-3">
                <div class="col-12 col-md-8">
                    <label class="form-label h6 mb-1" for="<?= e($categoriesSearchId) ?>">Search</label>
                    <input
                        id="<?= e($categoriesSearchId) ?>"
                        type="search"
            ...
            <div class="small text-muted mb-2" id="<?= e($categoriesCountId) ?>"></div>
            <div class="table-responsive">
                <table
                    id="<?= e($categoriesTableId) ?>"
                ...
                >
                    ...
                    <?php foreach ($categories as $category): ?>
                        ...
                        $categoryPagesUrl = $panelBase . '/pages?category=' . rawurlencode((string) $categoryId);
                        ?>
                        <tr
                        ...
                        >
                            ...
                            <td>
                                ...
                                <a href="<?= e($panelBase) ?>/categories/edit/<?= $categoryId ?>">
                                    <?= e($categoryName) ?>
                                </a>
                            </td>
                            ...
                            <td class="text-center">
                                <div class="d-flex justify-content-center gap-2">
                                    ...
                                    <form method="post" action="<?= e($panelBase) ?>/categories/delete" onsubmit="return confirm('Delete this category? Existing page-category links will be removed.');">
                                        <?= $csrfField ?>
            ...
            <p id="<?= e($categoriesEmptyId) ?>" class="text-muted mb-0 mt-2 d-none">No categories match the current filters.</p>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        var table = document.getElementById('<?= e($categoriesTableId) ?>');
        var searchInput = document.getElementById('<?= e($categoriesSearchId) ?>');
        var countLabel = document.getElementById('<?= e($categoriesCountId) ?>');
        var emptyLabel = document.getElementById('<?= e($categoriesEmptyId) ?>');
