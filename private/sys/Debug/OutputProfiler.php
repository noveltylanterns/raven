...
/**
 * Output-profiler entrypoint for arming response hooks and rendering toolbar HTML.
 */
final class OutputProfiler
{
    /**
    ...
    public static function arm(
        ...
        ob_start(static function (string $body) use ($settings, $scope, $requestMethod, $requestPath, $canRenderProfiler): string {
            ...
            return self::inject($body, $profilerHtml);
        });
    }
...
(function(){
    ...
    private static function renderQueries(array $queries, int $droppedCount): string
    {
        ...
        foreach ($queries as $query) {
            ...
            $bindingsText = is_array($bindings) && $bindings !== []
                ? OutputProfilerSanitizer::prettyJson($bindings)
                : '[]';
    ...
    {
        $payload = [
            '_GET' => OutputProfilerSanitizer::sanitizeArray($_GET),
            '_POST' => OutputProfilerSanitizer::sanitizeArray($_POST),
            '_FILES' => OutputProfilerSanitizer::sanitizeArray(OutputProfilerSanitizer::normalizeFiles($_FILES)),
            '_COOKIE' => OutputProfilerSanitizer::sanitizeArray($_COOKIE),
