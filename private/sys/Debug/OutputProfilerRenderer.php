...
/**
 * RAVEN CMS
 * ~/private/sys/Debug/OutputProfilerRenderer.php
 * HTML renderer and injector for the fixed-bottom output profiler toolbar.
...
/**
 * Builds output profiler toolbar markup and injects it into HTML responses.
 *
 * Combines markup generation and response injection so callers only depend
 * on one class. All markup is built inline; no sub-builder is needed.
 */
final class OutputProfilerRenderer
{
    /**
     * Builds the full profiler toolbar HTML from a captured request snapshot.
     *
     * Returns an empty string when the calling context does not have
     * system-configuration permission — fail-closed to avoid leaking profiler
     * data to unprivileged users.
     *
     * @param array{
     ...
     * @return string Rendered toolbar HTML for injection, or empty string when access is denied.
     */
    public static function render(array $settings, array $profile, array $context): string
    {
        // Fail closed: only emit markup when caller confirms current user has
        // system-configuration permission, preventing data leaks to other roles.
...
(function(){
     ...
     *
     * Redirects (3xx) and empty bodies are excluded. Non-HTML content types are
     * excluded when an explicit Content-Type header is present.
     *
     ...
     *
     * Falls back to appending after the body content when no closing tag is found,
     * so the toolbar always appears even on partial or malformed HTML responses.
     *
     * @param string $body Current buffered response body.
     * @param string $profilerHtml Output profiler markup to inject.
     * @return string Response body with toolbar markup inserted before </body>.
     */
    ...
    private static function renderQueries(array $queries, int $droppedCount): string
    {
        ...
        foreach ($queries as $query) {
            ...
            $bindingsText = is_array($bindings) && $bindings !== []
                ? OutputProfilerDataSanitizer::prettyJson($bindings)
                : '[]';
    ...
    {
        $payload = [
            '_GET' => OutputProfilerDataSanitizer::sanitizeArray($_GET),
            '_POST' => OutputProfilerDataSanitizer::sanitizeArray($_POST),
            '_FILES' => OutputProfilerDataSanitizer::sanitizeArray(OutputProfilerDataSanitizer::normalizeFiles($_FILES)),
            '_COOKIE' => OutputProfilerDataSanitizer::sanitizeArray($_COOKIE),
            '_SERVER' => OutputProfilerDataSanitizer::sanitizeServer($_SERVER),
        ];

        return '<pre>' . self::e(OutputProfilerDataSanitizer::prettyJson($payload)) . '</pre>';
    }
    ...
    private static function renderEnvironment(string $scope, string $hostname, string $requestPath): string
    {
        ...
        return '<pre>' . self::e(OutputProfilerDataSanitizer::prettyJson($info)) . '</pre>';
    }
