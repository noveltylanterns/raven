 ...
 * Admin panel group-listing template.
...
<form id="<?= e($bulkDeleteFormId) ?>" method="post" action="<?= e($panelBase) ?>/group/delete">
    <?= $csrfField ?>
</form>
...
>
    ...
    <?php foreach ($groups as $group): ?>
        ...
        >
            ...
            <td><?= $groupId ?></td>
            <td>
                <a href="<?= e($panelBase) ?>/group/edit/<?= $groupId ?>">
                    <?= e($groupName) ?>
                </a>
            </td>
            <td>
                <?php if ($memberCount > 0): ?>
                    <a
                        href="<?= e($panelBase) ?>/user?group=<?= rawurlencode(strtolower($groupName)) ?>"
            ...
            <td class="text-center">
                <div class="d-flex justify-content-center gap-2">
                    <a
                        ...
                        href="<?= e($panelBase) ?>/group/edit/<?= $groupId ?>"
                        title="Edit"
                        aria-label="Edit"
                    >
...
<?= ListWrapper::render([
    ...
    'search_placeholder'  => 'Filter by ID or name...',
    'filters'             => [
        [
            'id'           => $groupsTypeFilterId,
            'label'        => 'Type',
            'col'          => 'col-12 col-md-4',
            'options_html' => '<option value="">All Types</option><option value="stock">Stock</option><option value="custom">Custom</option>',
        ],
    ],
    'count_id'            => $groupsCountId,
    'empty_id'            => $groupsEmptyId,
    'empty_match_message' => 'No groups match the current filters.',
    'table_html'          => $groupsTableHtml,
    'pagination'          => [
        'current'     => max(1, (int) ($pagination['current'] ?? 1)),
        'total_pages' => max(1, (int) ($pagination['total_pages'] ?? 1)),
        'total_items' => max(0, (int) ($pagination['total_items'] ?? count($groups))),
        'base_path'   => (string) ($pagination['base_path'] ?? ($panelBase . '/group')),
        'query'       => is_array($pagination['query'] ?? null) ? $pagination['query'] : [],
        'label'       => 'groups',
        'aria_label'  => 'Groups pagination',
    ],
]) ?>
