...
/**
 * RAVEN CMS
 * ~/private/ext/signups/tpl/panel_edit.php
 * Signup Sheets extension edit/create page template.
...
$shortcodeValue = $formSlug !== '' ? '[signups slug="' . $formSlug . '"]' : '';
...
<form method="post" action="<?= e($formAction) ?>">
    ...
    <section class="card">
        <div class="card-body">
            <h2>Meta</h2>
            <div class="mb-3">
                <label class="form-label" for="signups_form_name">Name</label>
                <input
                    id="signups_form_name"
                    type="text"
                    class="form-control"
                    name="name"
                    required
            ...
            <div class="mb-3">
                <label class="form-label" for="signups_form_slug">Slug</label>
                <input
                    id="signups_form_slug"
                    type="text"
                    class="form-control"
                    name="slug"
                    required
        ...
        <div class="card-body">
            <div class="mb-3">
                ...
                <div class="form-text mb-2">
                    Default fields are always included: Email, Display Name, and Country.
                    Add optional additional fields below.
                </div>
                <div id="signups-additional-fields">
                    <?php foreach ($additionalFields as $index => $additionalField): ?>
                        ...
                        $fieldOptionsInputRaw = $additionalField['options_input'] ?? null;
                        $fieldOptionsRaw = $additionalField['options'] ?? [];
                        if (is_scalar($fieldOptionsInputRaw)) {
                            $fieldOptionsValue = (string) $fieldOptionsInputRaw;
                            $fieldOptionsValue = str_replace("\0", '', $fieldOptionsValue);
                            $fieldOptionsValue = str_replace(["\r\n", "\r"], "\n", $fieldOptionsValue);
                            $fieldOptionsValue = trim($fieldOptionsValue);
                        } elseif (is_string($fieldOptionsRaw)) {
                            $fieldOptionsValue = str_replace("\0", '', $fieldOptionsRaw);
                            $fieldOptionsValue = str_replace(["\r\n", "\r"], "\n", $fieldOptionsValue);
                            $fieldOptionsValue = trim($fieldOptionsValue);
                        } else {
                            $fieldOptions = [];
                            if (is_array($fieldOptionsRaw)) {
                                foreach ($fieldOptionsRaw as $fieldOptionRaw) {
                                    if (!is_scalar($fieldOptionRaw)) {
                                        continue;
                                    }

                                    $fieldOption = trim((string) $fieldOptionRaw);
                                    if ($fieldOption === '' || isset($fieldOptions[$fieldOption])) {
                                        continue;
                                    }
                            ...
                            $fieldOptionsValue = implode("\n", array_values($fieldOptions));
                        }
                        $showOptionsForType = in_array($fieldType, ['radio', 'checkbox', 'select'], true);
                        ?>
                        <div class="<?= e($customFieldBlockLayout['row_class']) ?>" data-signups-additional-row="1">
                            <div class="row g-2 align-items-end">
                                    ...
                                    <div class="input-group">
                                        <span class="input-group-text gap-2">
                                            <input
                                                ...
                                                type="checkbox"
                                                value="1"
                                                data-signups-field-key="required"
                                ...
                                <div class="col-auto ps-md-0 d-flex align-items-end">
                                    <button
                                        ...
                                        data-signups-remove-field="1"
                                        aria-label="Remove field"
                                        title="Remove field"
                                        style="min-height: calc(1.5em + 0.75rem + 2px);"
                                    ><i class="bi bi-x-circle-fill" aria-hidden="true"></i></button>
                                </div>
                                <div class="col-12<?= $showOptionsForType ? '' : ' d-none' ?>" data-signups-field-options-wrap="1">
                                    <label class="form-label">Options</label>
                                    <textarea
                                        class="form-control"
                                        data-signups-field-key="options"
                                        name="additional_fields[<?= (int) $index ?>][options]"
                                        rows="2"
                                        placeholder="Option A, Option B"
                                    ><?= e($fieldOptionsValue) ?></textarea>
                                    <div class="form-text">Used by Radio, Checkbox, and Dropdown fields. Enter one per line or comma-separated.</div>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
                <button type="button" class="btn btn-primary btn-sm" id="signups-additional-field-add">Add Field</button>
            </div>

            <div class="form-check mb-0">
                <input
                    id="signups_form_enabled"
                    type="checkbox"
                    ...
                    <?= $formEnabled ? 'checked' : '' ?>
                >
                <label class="form-check-label" for="signups_form_enabled">Enabled</label>
            </div>
        </div>
    </section>
    ...
    <div class="<?= e($customFieldBlockLayout['row_class']) ?>" data-signups-additional-row="1">
        <div class="row g-2 align-items-end">
            <div class="col-md-4">
                ...
                <div class="input-group">
                    <span class="input-group-text gap-2">
                        <input class="form-check-input mt-0" type="checkbox" value="1" data-signups-field-key="required" aria-label="Required">
                        <span class="small">Required</span>
                    </span>
                    <select class="form-select" data-signups-field-key="type">
            ...
            <div class="col-12 d-none" data-signups-field-options-wrap="1">
                <label class="form-label">Options</label>
                <textarea
                    class="form-control"
                    data-signups-field-key="options"
...
<script>
  (function () {
    ...
    function updateShortcode() {
      ...
      shortcodeElement.textContent = '[signups slug="' + slug + '"]';
    }
...
$editorBlocksBoot = [
    [
        'list_id' => 'signups-additional-fields',
        'template_id' => 'signups-additional-field-template',
        'add_button_id' => 'signups-additional-field-add',
        'row_selector' => '[data-signups-additional-row="1"]',
        'remove_selector' => '[data-signups-remove-field="1"]',
        'fields' => [
            ['selector' => '[data-signups-field-key="required"]', 'name_prefix' => 'additional_fields[{index}]', 'key_attribute' => 'data-signups-field-key'],
            ['selector' => '[data-signups-field-key="type"]', 'name_prefix' => 'additional_fields[{index}]', 'key_attribute' => 'data-signups-field-key'],
            ['selector' => '[data-signups-field-key="label"]', 'name_prefix' => 'additional_fields[{index}]', 'key_attribute' => 'data-signups-field-key'],
            ['selector' => '[data-signups-field-key="name"]', 'name_prefix' => 'additional_fields[{index}]', 'key_attribute' => 'data-signups-field-key'],
            ['selector' => '[data-signups-field-key="options"]', 'name_prefix' => 'additional_fields[{index}]', 'key_attribute' => 'data-signups-field-key'],
        ],
        'reindex_sync' => [
            [
                'action' => 'toggle-options',
                'select_selector' => '[data-signups-field-key="type"]',
                'wrap_selector' => '[data-signups-field-options-wrap="1"]',
            ],
        ],
        'change_sync' => [
            [
                'action' => 'toggle-options',
                'selector' => '[data-signups-field-key="type"]',
                'select_selector' => '[data-signups-field-key="type"]',
                'wrap_selector' => '[data-signups-field-options-wrap="1"]',
            ],
        ],
