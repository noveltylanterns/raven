...
final class SignupPublicFormRuntime implements EmbeddedFormRuntimeInterface
{
    ...
    public function render(array $definition, string $returnPath, string $csrfField, string $captchaMarkup): string
    {
        ...
        foreach ($this->additionalFieldDefinitions($rawSlug, $definition) as $fieldDefinition) {
            ...
            if ($fieldType === 'radio' && $fieldOptions !== []) {
                ...
                foreach ($fieldOptions as $optionIndex => $optionValueRaw) {
                    ...
                    $inputId = htmlspecialchars('signups-' . $rawSlug . '-' . $fieldName . '-option-' . $optionIndex, ENT_QUOTES, 'UTF-8');
            ...
            if ($fieldType === 'checkbox') {
                ...
                $inputId = htmlspecialchars('signups-' . $rawSlug . '-' . $fieldName, ENT_QUOTES, 'UTF-8');
        ...
        return '<section class="raven-embedded-form raven-embedded-form-signups" id="' . $sectionId . '" data-rvn-form-type="signups" data-rvn-form-slug="' . $slug . '">'
            . $flashMarkup
            . '<form method="post" action="' . $submitAction . '" novalidate>'
            . $csrfField
            . '<input type="hidden" name="_rvn_form_type" value="signups">'
            . '<input type="hidden" name="_rvn_form_slug" value="' . $slug . '">'
            . '<input type="hidden" name="return_path" value="' . $safeReturnPath . '">'
            . '<div class="row g-3">'
            . '<div class="col-12"><label class="form-label">Name</label><input type="text" class="form-control" name="signups_display_name" placeholder="Your Name" value="' . $oldValues['display_name'] . '" required></div>'
     ...
     *
     * @param array{email?: string, display_name?: string, country?: string, additional?: array<string, string|array<int, string>>} $old
     */
     ...
     *   type: string,
     *   message: string,
     *   old: array{email: string, display_name: string, country: string, additional: array<string, string|array<int, string>>}
     * }|null
     */
