...
| `Raven\Lib\View\Public\Meta::absoluteMetaImageUrl()` | `method` | Resolves a configured or stored image path into an absolute meta-image URL. | string Absolute meta-image URL, or an empty string when unusable. |
...
| `writeRelativeSymlink()` | `function` | Writes or replaces one relative symlink. | void |
...
- File: `private/lib/View/Public/Meta.php`
- Params:
  - `string $value Candidate configured URL or local path.`
...
- File: `private/lib/View/Public/ThemeGenerator.php`
- Params:
  - `string $linkPath Absolute link path to create.`
  - `string $target Relative symlink target.`
