...
| `Raven\Core\Repository\ChannelRead::listParentOptions()` | `method` | Returns root-first hierarchical options for the channel parent selector. | array<int, array{id: int, name: string, slug: string, parent_id: int, depth: int}> Root-first indented parent options. | `(none)` |
...
| `Raven\Core\Repository\PageRead::channelHomepagesForRouting()` | `method` | Returns one landing-page slug map keyed by channel slug for routing inventory. | array<string, string> Channel slug to homepage slug map; empty string means no homepage found. | `(none)` |
...
| `Raven\Core\Repository\PageRead::findChannelHomepage()` | `method` | Finds the channel homepage by slug priority: `home` first, then `index`. | array{channel: array<string,mixed>, page: ?array<string,mixed>}\|null Null when channel not found. | `(none)` |
...
- Service Keys: `(none)`
- Params:
  - `string $channelSlug Normalized channel slug to look up.`
...
- Params:
  ...
  - `string|null $channelSlug Optional channel slug to scope the lookup.`
...
- Params:
  ...
  - `string|null $channelSlug Optional channel slug scope; null matches root redirects.`
