...
- `private/sys/Controller/`
  ...
  - Split panel sub-controllers now live under `private/sys/Controller/Panel/` with a shared `RequestContext`; `DashboardController`, `TaxonomyController`, and `UserController` already own the `/`, `/redirect*`, and `/user*` seams while the legacy flat `PanelController.php` still owns the remaining unsplit panel routes during the migration.
