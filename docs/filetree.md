...
- `private/lib/Scribe/`
  ...
  - `ChannelScribe` is a low-level channel-file helper; repository-facing channel file writes are surfaced through `ChannelWrite` static methods (`normalizeStorageLayout`, `writeRecordById`, `deleteRecordById`, `persistChannelId`).
  - `MediaScribe` owns panel meta-image filesystem writes. `MediaRead` keeps read/list/public-gallery queries, while `MediaWrite` now owns page-gallery SQL write paths.
