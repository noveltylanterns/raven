...
- `private/lib/Parser/`
  ...
  - `ConfigParser` is now a compatibility wrapper; the canonical config read helpers (dot-path reads, scalar coercion, nested-form reads, and config-field stringification) live in `private/sys/Repository/ConfigRead.php`. `PanelParser` owns panel-path normalization only. `RoutePrefixParser` owns generic route-prefix slug normalization used by feed/category/tag/group route policy parsers. These are utility parsers with no route/data split.
- `private/lib/Scribe/`
  - Canonical write-side helpers that pair with the parser layer.
  - `ConfigScribe` is now a compatibility wrapper; canonical nested config writes and persistence now live in `private/sys/Repository/ConfigWrite.php`.
  - `ChannelScribe` is now a low-level compatibility primitive; repository-facing channel file writes are surfaced through `ChannelWrite` static methods (`normalizeStorageLayout`, `writeRecordById`, `deleteRecordById`, `persistChannelId`).
  ...
  - `CategoryScribe` and `TagScribe` are now compatibility layers; category/tag SQL mutation seams are owned directly by `CategoryWrite` and `TagWrite` (save/update, taxonomy-image filename persistence, default-set reassignment, and transactional delete-plus-page-detach cleanup).
  - `SetScribe` is now a compatibility layer; low-level set-file writes and layout repair are exposed through `SetWrite` static helpers (`normalizeStorageLayout`, `writeRecordById`, `deleteRecordById`, `ensureRootRecord`) and used by `SetRead`/`SetWrite`.
  - `RedirectScribe` is now a compatibility layer; redirect SQL writes are owned directly by `RedirectWrite` (channel-scope resolution, `(channel, slug)` uniqueness enforcement, create/update persistence, delete-by-id cleanup).
  - `GroupScribe` is now a compatibility layer; group SQL mutation rules are owned directly by `GroupWrite` (stock-role save policy, custom id allocation, image filename writes, guarded delete behavior).
  - `UserScribe` now primarily serves cross-callsite user media/file helpers; user SQL writes are owned directly by `UserWrite`.
  - `AuthScribe` owns auth-user profile/security writes for existing accounts: current-user preference updates, password changes, avatar/cover references, and stored 2FA payload persistence. `AuthService` keeps the login/session/read facade above it. `AuthProfileScribe` remains a compatibility alias for older imports.
