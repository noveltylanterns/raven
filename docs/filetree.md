...
- `private/lib/Security/`
  - Security primitives available to core and extensions: CSRF (`Csrf`, `CsrfToken`), input sanitization (`InputSanitizer`), user-string generation (`UserString`), 2FA method primitives (`Totp`, `TotpCipher`, `WebAuthn`, `RecoveryPhrase`, `TwoFactorMethodKey`, `TwoFactorMethodNormalizer`, `TwoFactorMethodRules`), and captcha (`Captcha`).
- `private/lib/Extra/`
