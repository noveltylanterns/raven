...
- `private/lib/Scribe/`
  ...
  - `MediaScribe` owns page-gallery media writes: source-plus-variant inserts, cover-selection normalization, page-media metadata updates, and transactional delete cleanup for media/variant rows. `MediaRead` keeps the read/list/public-gallery queries; `MediaWrite` owns the write paths.
