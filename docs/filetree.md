...
- `private/sys/Repository/`
  ...
  - Write classes: `PageWrite`, `ChannelWrite`, `UserWrite`, `GroupWrite`, `CategoryWrite`, `TagWrite`, `SetWrite`, `RedirectWrite`, `PageImageWrite`, `InviteWrite`. Each `*Write` takes the corresponding `*Read` as a constructor arg for validation lookups.
  ...
  - `PageImageRepository` bridge remains as a justified holdout for `PageImageManager`, which needs both read and write operations in one object until it is refactored to accept split injections.
- `private/sys/Routing/`
...
- `private/lib/Scribe/`
  ...
  - `PageImageScribe` owns page-gallery image writes: source-plus-variant inserts, cover-selection normalization, page-image metadata updates, and transactional delete cleanup for image/variant rows. `PageImageRead` keeps the read/list/public-gallery queries; `PageImageWrite` owns the write paths.
  ...
  - `TaxonomyScribe` owns the shared SQL write paths for category/tag rows: save/update, taxonomy-image filename persistence, default-set reassignment, and transactional delete-plus-page-detach cleanup. `CategoryRead`/`TagRead` keep the read-heavy listing and lookup queries; `CategoryWrite`/`TagWrite` own the write paths.
