...
- `private/sys/Factory/`
  - Runtime payload factory contracts and assertion helpers used while extracting scope builders out of `sys/Routing`.
...
- `private/sys/Routing/`
  ...
  - `RouteParamGuard.php` — shared route-param guard helpers (`slugOrNotFound`, `intOrNotFound`, `slugAllowedOrNotFound`) used by public/panel route registrars to keep validation/404 behavior consistent.
  - `Routing/Public/` — `PublicRuntimeBuilder` plus `PublicRouter` (scope-owned public orchestration over an isolated internal `Router` instance), controller-aligned public route registrars (including extension-route loading), shared registrar deps payload `RouteDeps` (used by `registerWithDeps(...)` adapters to reduce positional registrar signatures), `RouteConfig`, `PrefixedSlugPageRouter` (shared slug+pagination route builder used by `CategoryRouter` and `TagRouter`), and `ChannelPageRouter` (wraps `Raven\Lib\Parser\ModeParser` for public content controllers: lookup-target resolution and canonical segment building).
  - `Routing/Panel/` — `PanelRuntimeBuilder` plus `PanelRouter` (scope-owned panel orchestration over an isolated internal `Router` instance), controller-aligned panel route routers (including dedicated `ChannelRouter`, `CategoryRouter`, `TagRouter`, `LogRouter`, `RoutingRouter`, and `UpdateRouter` files for the split panel route families), `TaxonomyCrudRouter` (shared 10-route CRUD+set builder used by `CategoryRouter` and `TagRouter`), shared registrar deps payload `RouteDeps` (used by `registerWithDeps(...)` adapters to reduce positional registrar signatures), `NavSessionPopulator` (owns all `$_SESSION['_raven_nav_*']` and `$_SESSION['_raven_extension_*']` population extracted from `panel/index.php`), extension-route loading, panel theme-asset fast path, and `RoutingInventoryBuilder` (builds the normalized routing inventory row set for the panel routing diagnostics view).
