...
- `private/sys/Router/`
  ...
  - `UserPolicy.php` — user profile routing policy (config-backed instance class): `profileRoutePrefix`, `profileSelector`, `profileMode`, `profileRoutesEnabledForRoutingTable`, `registrationMode`.
...
- `private/lib/Parser/`
  ...
  - `PanelParser` owns panel-path normalization. Routing policy helpers (`ChannelPolicy`, `PagePolicy`, `CategoryPolicy`, `TagPolicy`, `FeedPolicy`, `UserPolicy`, `GroupPolicy`) have moved to `sys/Router/`. Channel and page repo-shared primitives (`ChannelShared`, `PageShared`) have moved to `sys/Repository/`.
- `private/lib/Scribe/`
